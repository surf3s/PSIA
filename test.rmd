...
```{r fig-core_outline, fig.margin = TRUE, fig.cap = "Outline of Dible core surface.", fig.width=3.5, fig.height=2.5, cache=TRUE}
...
```{r fig-platform_outline, fig.margin = TRUE, fig.cap = "The platform edge (from outline shown in previous figure) resampled.", fig.width=3.5, fig.height=1.5, cache=TRUE}
...
```{r fig-test_1_angles, fig.cap = "Distribution of estimated platform surface interior angles (PSIA) based on the Dibble glass flakes.", cache=TRUE}
...
Figure X shows the distribution of platform surface interior angles in the Dibble glass dataset.  The distribution has a mean of `r round(mean(angles),2)` +-  `r round(sd(angles),2)`.  Variation in this angle does not seem to be related to platform depth, exterior platform angle or mass (Figure X+1).  There is a relationship between platform width and the platform surface interior angle such that larger angles result in wider platforms (see Figure X+1).  

```{r fig-angles_to_other_measures, fig.fullwidth = TRUE, fig.height = 2, fig.cap = "PW, PD, EPA and mass against the estimated platform surface interior angles based on the Dibble glass flakes.", cache=TRUE}
...
p1 = p1 + ylab('PSIA')
p1 = p1 + xlab('Platform Depth')
p1 = p1 + my_theme
p1 = p1 + full_width_theme

p2 = ggplot()
p2 = p2 + geom_point(plot_data, mapping = aes(pw, angles), size = .25)
p2 = p2 + ylab('PSIA')
p2 = p2 + xlab('Platform Width')
p2 = p2 + my_theme
p2 = p2 + full_width_theme
...
There also appears to be a relationship between the angle of blow and the platform surface interior angle (Figure X+2).  While there are fewer cases with angles of blow less than 20, there is some indication in the data that lower angles of blow are correlated with lower platform surface interior angles.  After an angle of blow of 20 degrees, however, the angle of blow is not correlated with the platform surface interior angle.
...
```{r fig-AOB_to_PSIA, fig.height = 2, fig.cap = "AOB against the estimated platform surface interior angles based on the Dibble glass flakes.", cache=TRUE}

p4 = ggplot()
p4 = p4 + geom_point(plot_data, mapping = aes(AOB, angles), size = .25)
p4 = p4 + ylab('PSIA')
p4 = p4 + xlab('AOB')
p4 = p4 + my_theme
p4 = p4 + maincolumn_width_theme
p4
...
```{r fig-pd_pw_with_estimated_pw, fig.height = 2, fig.cap = "The actual PD to PW data from the Dibble glass core flakes and the estimated PW using the average PSIA computed from Test 1.", cache=TRUE}
...
p = p + geom_point(size = .25)
p = p + xlab('Platform Depth')
p = p + ylab('Platfrom Width')
p = p + my_theme
p = p + maincolumn_width_theme
p
...
```{r figure_remeasure_distributions, fig.cap = 'The distributions of platform surface interior angle as directly measured with a goniometer and as calculated from caliper measurements.'}
...
The distribution of the platform surface interior angles for the  `r nrow(data)` flakes in the Campagne dataset is shown in Figure X+4 with color coding for the type of percussion technique.  Here the mean platform surface interior angle for all flakes is `r round(mean(data$angles),2)` +- `r round(sd(data$angles),2)`.  Punch flakes have a lower platform surface interior angle (stats), and softhammer flakes have a mean of .  When we look at only the hard hammer flakes, which is the technique used in the Dibble glass dataset, the mean platform surface interior angle is `r round(mean(data %>% filter(Technique=='Hard Hammer') %>% select(angles) %>% pull()),2)` +- `r round(sd(data %>% filter(Technique=='Hard Hammer') %>% select(angles) %>% pull()),2)`.



```{r fig-angles, fig.cap = "Distribution of platform surface interior angles.", cache=TRUE}
...
In the Campagne data, platform surface interior angle does not covary with platform width, platform thickness or the shape of the platform (PW/PD).  Though sample size is perhaps a problem, there is perhaps an indication that for larger platform depths, there is less variability in the platform surface interior angle.

```{r fig-PSIA_to_other_measaures, fig.cap = "PSIA as a function of platform width, platform depth, and EPA in the experimental flake collection.  Color coding is the same as previous figure."}
...
The distribution of platform surface interior angles in the MPI dataset as measured by a digital goniometer is presented in Figure X.  This dataset contains ```r nrow(MPI_flakes)``` flakes, and the mean is `r round(mean(MPI_flakes$PSIA_GONIOMETER),2)` +- `r round(sd(MPI_flakes$PSIA_GONIOMETER),2)`, in keeping with the other datasets.


```{r marcel_figure, fig.cap = 'Distribution of platform surface interior angle in the MPI flakes dataset as measured by a digital goniometer.'}
...
model_mass = lm(WEIGHT ~ PW + EPA + PW * EPA, data = expdata)

```

Beveled flakes are ones where material is removed behind the platform prior to striking the core.  Dibble recognized that beveling altered the EPA-PD model of flake formation model such that the interaction of platform depth and exterior platform angle no longer predicted flake size (Leader et al.).  Beveled flakes have too thin a platform for their size.  However, the inclusion of platform surface interior angle pulls beveled flakes back into the model.  Too illustrate this point, we examine some of the beveled flakes in the Dibble glass dataset.

In the Dibble glass dataset there are `r nrow(bevel_flakes)` beveled flakes coming from semispherical cores and otherwise conforming to our selection criteria.  They are plotted in Figure X along with the non-beveled flakes to illustrate how the beveling changes the relationship of platform thickness to platform width.  To illustrate the power of platform surface interior angle to explain these flakes, we can build a linear model to predict platform thickness from the platform surface interior angle and platform width on the non-beveled flakes in the Dibble glass dataset.  We then use this model to calculate the platform thickness of the beveled flakes.  However, the platform surface interior angle is not known for these flakes, and so we substitute the average PSIA in its place.  When this is done, the platform thickness for the beveled flakes plot on the same trend line as the non-beveled flakes (see Figure).  

This reconstructed platform thickness should then give a better estimate of flake size.  We demonstrate this by first modeling flake weight as a function of exterior platform angle, platform thickness, and the interaction of two in the non-beveled flakes and plot the actual weight against the predicted weight (see Figure).  Next, we use apply this model using the reconstructed platform thickness to predict the weight of the beveled flakes (see Figure).

```{r fig-bevel_with_estimated_pd, fig.cap = "Platform thickness to platform width including beveled flakes.  Estimated points are the beveled flakes with recalculated platform thicknesses based on the average PSIA and their actual platform width."}

p1 = ggplot(beveldata, aes(PD, PW, color = BEVEL))
p1 = p1 + geom_point(size = .25)
p1 = p1 + geom_point(bevel_flakes, mapping = aes(PD, PW, color = 'Estimated'), size = .25)
p1 = p1 + xlab('Platform Thickness')
p1 = p1 + ylab('Platform Width')
p1 = p1 + my_theme
p1 = p1 + maincolumn_width_theme

p2 = ggplot(beveldata, aes(WEIGHT, predict(model_mass, data.frame(beveldata)), color = BEVEL))
p2 = p2 + geom_point(size = .25)
p2 = p2 + geom_point(bevel_flakes, mapping = aes(WEIGHT, predict(model_mass, bevel_flakes), color = 'Estimated'), size = .25)
p2 = p2 + xlab('Actual Weight (gm)')
p2 = p2 + ylab('Predicted Weight (gm)')
p2 = p2 + my_theme
p2 = p2 + maincolumn_width_theme
#p2 = p2 + coord_fixed()
...
We note that our finding that the platform surface interior angle varies around a constant derived from fracture mechanics appears to be consistent with all of the findings to date of the Dibble glass experiments ().  Additionally, it perhaps helps explain or account for one of the more counter intuitive finds of the glass experiments, namely that flake size (mass) is not impacted by the force with which the core is struck (Dibble and Rezek 2009).  In the EPA-PD model, the amount of force required to remove a flake given a particular combination of EPA-PD is a constant.  Subtracting force means that the flake is not initiated.  Adding force does not change how the shape (size and mass) of the resulting flake.  This makes sense in the PSIA addition to the model.  In fracture mechanics, it is known that striking a material harder does not change the angle of the cone.  It will change its size but not the angle.  Thus when a core is struck, how far into the core the Hertzian cone travels will be a function of force, but where it is destined to intersect the core surface does not change.  So striking a Dibble glass core harder at a particular point does not change the platform width, and as a result, the subsequent fracture plane that removes the flake has much less freedom to change the size of the flake.  
...
This said, our goal here was to return to this literature and to try to find some useful insights that could be translated to a better understanding of the underlying mechanisms (or first principals) of flake formation and that might thereby lead to a better integration of the various statistical models currently in use.  To do this, we discarded the existing focus in the experimental literature on what knappers do and instead focused on attributes that may be more directly related to fracture mechanics.  Thus we focused our attention on Hertzian percussion cone as a constant and wondered if it could help explain platform width, an aspect of flake size and shape that up to now has been absent from the dominant EPA-PD model of flake formation.  We measured three different collections in multiple different ways and found that in each case the angle formed by the platform width and the point of percussion to be, on average, essentially the same as what is predicted from fracture mechanics for the angle of the Hertzian cone. 
