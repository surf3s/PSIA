...
Figures 6 and 7 show comparisons of the results of the estimated PSIA presented above with direct measurements of this angle on a sample of `r nrow(Dibble_remeasure)` of the Dibble glass flakes.  For this sample, the PSIA is `r round(mean(Dibble_remeasure$psia),2)`±`r round(sd(Dibble_remeasure$psia),2)`.  When measured with a digital goniometer the angle is `r round(mean(Dibble_remeasure$PSIA_GONIOMETER),2)`±`r round(sd(Dibble_remeasure$PSIA_GONIOMETER),2)`, and when measured with digital calipers and calculated using trigonometry the angle is `r round(mean(Dibble_remeasure$psia_trig),2)`±`r round(sd(Dibble_remeasure$psia_trig),2)`.
...
The distribution of the PSIA for the  `r nrow(data)` flakes in the Campagne dataset is shown in Figure 8.  In the hard hammer flakes only, which is the technique used in our subset of the Dibble glass dataset, the mean platform surface interior angle is `r round(mean(data %>% filter(Technique=='Hard Hammer') %>% select(angles) %>% pull()),2)`±`r round(sd(data %>% filter(Technique=='Hard Hammer') %>% select(angles) %>% pull()),2)`.  Because data are available for punch and soft hammer flakes, these are presented as well.  Punch flakes have a lower PSIA (stats), and soft hammer flakes have a mean of XX.  The mean PSIA for all flakes in the Campagne data set is `r round(mean(data$angles),2)`±`r round(sd(data$angles),2)`. 
...
```{r fig9-PSIA_to_other_measaures, fig.height = 2, fig.width = 5, fig.cap = "PSIA as a function of platform width, platform depth, and EPA in the experimental flake collection.  Color coding is the same as in Figure 8."}
