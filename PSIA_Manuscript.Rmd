---
title: Introducing Platform Surface Interior Angle and Its Role in Flake Formation,
  Size and Shape
author:
- affiliation: MPI
...
- address: Department of Human Evolution, Max Planck Institute for Evolutionary Anthropology,
    Leipzig, Germany
  code: MPI
- address: Department of Anthropology, University of Pennsylvania, Philadelphia, PA,
    USA
  code: UPENN
...
```{r fig2-core_outlines, fig.cap = "Outline of Dibble core surface (left) and the polynominal fitted to the core edge (right).", fig.height = 2, fig.width = 5}
...
```{r fig3-dibble_psia_histogram, fig.cap = "Distribution of estimated PSIA based on the Dibble glass flakes.", fig.width = 5, fig.height = 2.5}
...
if (tiff_figures) {
    ...
    tiff('figure_03tif', res = 300, width = 5, height = 2.5, units = 'in') }
...
```{r fig4-psia_to_other_flake_measures, fig.cap = "PW, PD, EPA and weight against the estimated PSIA based on the Dibble glass flakes.", fig.width = 5}
...
```{r fig5-AOB_to_PSIA, fig.height = 2.5, fig.width = 4, fig.cap = "AOB against the estimated platform surface interior angles based on the Dibble glass flakes."}
...
```{r fig6-pd_pw_with_estimated_pw, fig.height = 2.5, fig.width = 5, fig.cap = "The actual platform depth to platform width data from the Dibble glass core flakes and the estimated platform width using the average platform surface interior angle calculated previously."}
...
```{r fig7_remeasure_comparisons, fig.height = 2, fig.width = 6.5, fig.cap = "A comparison of the estimated platform surface interior angle, this same angle as calculated from caliper measurements, and this same angle measured with a goniometer.  All measures are in degrees."}
...
```{r fig8_remeasure_distributions, fig.height = 2.5, fig.width = 6.5, fig.cap = 'The distributions of platform surface interior angle as directly measured with a goniometer and as calculated from caliper measurements.'}
...
```{r fig9-camp_angles, fig.width = 5, fig.height = 2.5, fig.cap = "Distribution of PSIA in the Campagne data set color coded by percussion type."}
...
```{r fig10-PSIA_to_other_measures, fig.height = 2, fig.width = 5, fig.cap = "PSIA as a function of platform width, platform depth, and EPA in the experimental flake collection.  Color coding is the same as in Figure 8."}
...
```{r fig11-mpi_data, fig.height = 2.5, fig.width = 5, fig.cap = 'Distribution of platform surface interior angle in the MPI flakes dataset as measured by a digital goniometer.'}
