...
address:
  ...
  - code: NMSA
    address: The National Museum of South Africa, Bloemfontein, South Africa
  - code: Leiden
  ...
  - code: UOW
    address: Centre for Archaeological Science, School of Earth, Atmospheric and Life Sciences, University of Wollongong, Wollongong, Australia
abstract: |
...
It requires:
1) the tidyverse, retistruct and gridExtra libraries,
2) the data sets XXXX (included here),
3) and the citations included here (bibliography.bib).
...
campagne = readRDS('Campagne_attribute_data.RDS')
...
if (tiff_figures) tiff('figure_01.tif', res = 300, width = 6.5, height = 6.5, units = 'in')
...
We use the R [@team_r_2010] statistical environment to do this analysis.  This paper is an rMarkdown document, and it is included in the supplementary information along with the data files needed to compile the document and replicate all of the figures, tables, and statistics. 
...
#dibble_extra = dibble
#dibble_extra$pd_bulb = pd_bulb
#dibble_extra$pd_edge = pd_edge
#dibble_extra$psia = angles
#dibble_extra$max_pop_2_pw = max_pop_2_pw

#saveRDS(dibble_extra, 'data//dibble exp data with psia.RDS')
#write.csv(dibble_extra %>% select(UNIT, ID, psia), 'data/psia_dibble_flakes.csv')

#dibble_remeasured = merge(dibble_remeasured, dibble_extra, by = c('ID')) %>%
#  select(psia, psia_trig, PSIA_GONIOMETER)

dibble_remeasured = merge(dibble_remeasured, dibble %>% mutate(psia = angles), by = c('ID')) %>%
  select(psia, psia_trig, PSIA_GONIOMETER)
...
```{r fig2-test_1_angles, fig.cap = "Distribution of estimated PSIA based on the Dibble glass flakes.", fig.width = 5, fig.height = 2.5}
...
if (tiff_figures) tiff('figure_02.tif', res = 300, width = 6.5, height = 6.5, units = 'in')
...
Figure 2 shows the distribution of estimated PSIA in the Dibble glass dataset.  The distribution has a mean of `r round(mean(angles),2)`±`r round(sd(angles),2)`.  Variation in this angle does not seem to be related to platform depth, exterior platform angle or weight (Figure 3).  There is a relationship between platform width and the platform surface interior angle such that larger angles result in wider platforms, which is to be expected (see Figure 3).  

```{r fig3-angles_to_other_measures, fig.cap = "PW, PD, EPA and weight against the estimated PSIA based on the Dibble glass flakes.", fig.width = 5}

full_width_theme = theme(text = element_text(size = 6))

plot_data = data.frame(cbind(pd, pw, EPA = dibble$EPA, mass = dibble$WEIGHT,  AOB = dibble$AOB, angles))
...
if (tiff_figures) tiff('figure_03.tif', res = 300, width = 6.5, height = 6.5, units = 'in')
...
There also appears to be a relationship between the angle of blow and the PSIA (Figure 4).  While there are fewer cases with angles of blow less than 20, there is some indication in the data that lower angles of blow may be correlated with lower PSIA.  However, once an angle of blow is between 10-20 degrees or higher, the angle of blow is not correlated with the PSIA.
...
if (tiff_figures) tiff('figure_04.tif', res = 300, width = 6.5, height = 6.5, units = 'in')
...
if (tiff_figures) tiff('figure_05.tif', res = 300, width = 6.5, height = 6.5, units = 'in')
...
Figures 6 and 7 show comparisons of the results of the estimated PSIA presented above with direct measurements of this angle on a sample of `r nrow(dibble_remeasured)` of the Dibble glass flakes.  For this sample, the PSIA is `r round(mean(dibble_remeasured$psia),2)`±`r round(sd(dibble_remeasured$psia),2)`.  When measured with a digital goniometer the angle is `r round(mean(dibble_remeasured$PSIA_GONIOMETER),2)`±`r round(sd(dibble_remeasured$PSIA_GONIOMETER),2)`, and when measured with digital calipers and calculated using trigonometry the angle is `r round(mean(dibble_remeasured$psia_trig),2)`±`r round(sd(dibble_remeasured$psia_trig),2)`.

```{r fig6_remeasure_comparisons, fig.height = 2, fig.width = 5, fig.cap = "A comparison of the estimated platform surface interior angle, this same angle as calculated from caliper measurements, and this same angle measured with a goniometer.  All measures are in degrees."}
...
if (tiff_figures) tiff('figure_06.tif', res = 300, width = 6.5, height = 6.5, units = 'in')
...
if (tiff_figures) tiff('figure_07.tif', res = 300, width = 6.5, height = 6.5, units = 'in')
...
The distribution of the PSIA for the `r nrow(campagne)` flakes in the Campagne dataset is shown in Figure 8.  In the hard hammer flakes only, which is the technique used in our subset of the Dibble glass dataset, the mean platform surface interior angle is `r round(mean(campagne %>% filter(Technique=='Hard Hammer') %>% select(angles) %>% pull()),2)`±`r round(sd(campagne %>% filter(Technique=='Hard Hammer') %>% select(angles) %>% pull()),2)`.  Because data are available for punch and soft hammer flakes, these are presented as well.  Punch flakes have a lower PSIA (`r round(mean(campagne %>% filter(Technique=='Punch') %>% select(angles) %>% pull()),2)`±`r round(sd(campagne %>% filter(Technique=='Punch') %>% select(angles) %>% pull()),2)`), and soft hammer flakes have a mean of `r round(mean(campagne %>% filter(Technique=='Soft Hammer') %>% select(angles) %>% pull()),2)`±`r round(sd(campagne %>% filter(Technique=='Soft Hammer') %>% select(angles) %>% pull()),2)`.  The mean PSIA for all flakes in the Campagne data set is `r round(mean(campagne$angles),2)`±`r round(sd(campagne$angles),2)`. 

```{r fig8-camp_angles, fig.width = 5, fig.height = 2.5, fig.cap = "Distribution of PSIA in the Campagne data set color coded by percussion type."}

p = ggplot(campagne, aes(angles, fill = Technique))
p = p + geom_histogram(binwidth = 5)
p = p + xlab('PSIA (degrees)')
p = p + ylab('Count')
p = p + my_theme

if (tiff_figures) tiff('figure_08.tif', res = 300, width = 6.5, height = 6.5, units = 'in')
...
In the Campagne data, PSIA does not covary with platform width, platform depth or the shape of the platform (as measured by the ratio of platform width to platform depth).  Though sample size is potentially a problem, there is perhaps an indication that for larger platform depths, there is less variability in the PSIA.

```{r fig9-PSIA_to_other_measures, fig.height = 2, fig.width = 5, fig.cap = "PSIA as a function of platform width, platform depth, and EPA in the experimental flake collection.  Color coding is the same as in Figure 8."}

p = ggplot(campagne, aes(angles, pw, color = Technique))
p = p + geom_point(show.legend = FALSE, size = .25)
p = p + xlab('PSIA')
p = p + ylab('Platform Width')
p1 = p + my_theme

p = ggplot(campagne, aes(angles, pd, color = Technique))
p = p + geom_point(show.legend = FALSE, size = .25)
p = p + xlab('PSIA')
p = p + ylab('Platform Depth')
p2 = p + my_theme

p = ggplot(campagne, aes(angles, pw/pd, color = Technique))
p = p + geom_point(show.legend = FALSE, size = .25)
p = p + xlab('PSIA')
p = p + ylab('PW/PD')
p3 = p + my_theme

if (tiff_figures) tiff('figure_09.tif', res = 300, width = 6.5, height = 6.5, units = 'in')
...
The distribution of PSIA in the MPI dataset as measured by a digital goniometer is presented in Figure 10.  This dataset contains ```r nrow(MPI_flakes)``` flakes, and the mean is `r round(mean(MPI_flakes$PSIA_GONIOMETER),2)`±`r round(sd(MPI_flakes$PSIA_GONIOMETER),2)`, in keeping with the other datasets.
...
if (tiff_figures) tiff('figure_10.tif', res = 300, width = 6.5, height = 6.5, units = 'in')
...
if (tiff_figures) tiff('figure_11.tif', res = 300, width = 6.5, height = 6.5, units = 'in')
